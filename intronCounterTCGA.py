...
for file, short_id in id_list:
	...
	for newline in intron_list:
		...
		for read in bamfile.fetch(chrom,start+5,end-5):
			if not read.is_duplicate and not read.is_qcfail and not read.is_secondary and read.is_proper_pair: #include XS tag for hisat2 aligned bams (and read.get_tag('XS') == strand)
				span_count+=1
